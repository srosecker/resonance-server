...
<aside class="
	...
	w-64 bg-mantle border-r border-border flex flex-col h-full
	transition-transform duration-300 ease-in-out shadow-2xl lg:shadow-none
	{uiStore.isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
">
	<!-- Logo Area -->
	<div class="h-[73px] px-6 flex items-center justify-between border-b border-border shrink-0">
		<div class="flex items-center gap-3">
			<div class="w-8 h-8 rounded-lg flex items-center justify-center color-transition dynamic-gradient"
	...
	<nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
		<div class="px-3 py-2 text-xs font-semibold text-overlay-0 uppercase tracking-wider mb-2">
			Menu
		</div>
		...
		{#each mainNavItems as item}
			...
			>
				<item.icon size={20} class="transition-colors {uiStore.currentView === item.view ? 'text-accent dynamic-accent' : 'group-hover:text-text'}" />
